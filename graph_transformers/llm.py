...
system_prompt = (
    "# 知识图谱构建指南\n"
    "## 1. 简介\n"
    "你是一个高级算法，专为从文本中提取信息并以结构化格式构建知识图谱而设计。尽可能准确地从文本中提取信息，不要添加文本中未明确提及的信息。\n"
    "- **实体** 代表实体和概念。\n"
...
#         "确保你使用基础或初级类型作为节点标签。\n"
#         "例如，当你识别出一个代表个人的实体时，"
#         "总是将其标记为 **'Person'**（人）。避免使用更具体的术语"
#         "如 'Mathematician'（数学家）或 'Scientist'（科学家）"
#     )
# elif input_type == "关系":
#     return (
#         "不要使用特定和临时的类型，如 'BECAME_PROFESSOR'（成为教授），"
#         "而应使用更普遍和持久的关系类型，如 'PROFESSOR'（教授）。"
#         "然而，不要为了普遍性而牺牲准确性"
#     )
